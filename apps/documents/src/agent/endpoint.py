        ...
        try:
            ...
            async for chunk in llm_with_tools.astream(lc_messages):
                ...
                collected_chunks.append({
                  "chunk": chunk,
                  "reasoning": [r for r in chunk.content_blocks if r.get('type') == 'reasoning'] if hasattr(chunk, 'content_blocks') else None,
                ...
                if chunk.content:
                    ...
                    full_response += chunk.content if isinstance(chunk.content, str) else json.dumps(chunk.content)
                    response_data["choices"][0]["delta"]["content"] = chunk.content
                    yield f'data: {json.dumps(response_data)}\n\n'
                            ...
                            if t.name == tool_name:
                                try:
                                    ...
                                    async for follow_chunk in llm.astream(lc_messages):
                                        ...
                                        collected_chunks.append({
                                            "content": follow_chunk.content if follow_chunk.content else None,
                                            "is_follow_up": True
                                        })
                                        if follow_chunk.content:
                                            # Ensure appending a string; convert non-str content to JSON
                                            full_response += follow_chunk.content if isinstance(follow_chunk.content, str) else json.dumps(follow_chunk.content)
                                            follow_data = {
                                                "choices": [{
                                                    "delta": {"content": follow_chunk.content}
                                                }]
                                            }
            ...
            if tool_calls_made:
                logger.info(f"Tool calls details: {json.dumps(tool_calls_made, indent=2)}")
